...
import {
  alertaRedireccion,
  alertGeneral, //alertError
  generarToken,
} from "../helpers/funciones"; // nos ahorramos lineas de codigo.
...
function Login() {
  ...
  useEffect(() => {
    getUsuarios();
  }, []); // del esta original de un objecto vacio , un estado por defecto
  //getUsuarios();
  ...
  function buscarUsuario() {
    ...
    ); // item es una variable de control puedes llamarlo tambien index o la forma de iniciar sesion de un usuario o un correo pondriamos ("||") and && getTipousuario=item.tipoUsuario extraido de la base de datos.
  ...
  function iniciarSesion() {
    if (buscarUsuario()) {
      ...
      alertGeneral("Error", "Error de credenciales", "error"); //alertError esto lo cambiamos
    }
  }
  ...
  function registrarUsuario() {
    ...
    if (auth) {
      alertGeneral("Error", "Error de credenciales", "error"); // alteramos el alerterror por esto. estamos cambiando desde funciones
    } else {
      let nuevoUsuario = {
        name: getName,
        correo: getEmail,
      ...
      fetch(apiUsuario, {
        ...
        alertGeneral(
          "REGISTRO EXITOSO",
          "INGRESE A LOGIN E INGRESAR CREDENCIALES ",
          "INFO"
        );
      });
    }
    ...
    <div className="container">
      ...
      <form className="form">
        <div className="form_front">
          ...
          <input
            onChange={(evtn) => setUser(evtn.target.value)}
            placeholder="Username"
            className="input"
            type="text"
          />
          {/* al onchange le mandamos una funcion flecha anonima, cuando detecte un cambio imprima e=element or evtn=evento*/}
          <input
            onChange={(evtn) => setPassword(evtn.target.value)}
            placeholder="Password"
            className="input"
            type="text"
          />
          {/*eliminamos el console log por setPassword. no queremos que solamente nos imprima pero que se visualice en la variable */}
          ...
          {/*aqui estamos agregando un evento onClick, type button para que no se recargue mi poryecto y no se reinicie la informacion*/}
        ...
        <div className="form_back">
          ...
          <input
            ...
            className="input"
            type="text"
            onChange={(evtn) => setFirstname(evtn.target.value)}
          />
          <input
            placeholder="Username"
            className="input"
          ...
          <input
            ...
            className="input"
            type="text"
            onChange={(evtn) => setEmail(evtn.target.value)}
          />
